...
function drawNextPiece(piece) {
  ...
  for (let i = 0; i < shape.length; i++) {
    for (let j = 0; j < shape[i].length; j++) {
      if (shape[i][j] === 1) {
        getElement(`cellNext-${row + i + 1}-${col + j - 2}`).classList.add(
          "active-piece"
        );
        getElement(`cellNext-${row + i + 1}-${col + j - 2}`).classList.add(
          color
...
function undrawPiece(piece) {
  if (piece) {
    ...
    for (let i = 0; i < shape.length; i++) {
      for (let j = 0; j < shape[i].length; j++) {
        if (shape[i][j] === 1) {
          getElement(`cell-${row + i}-${col + j}`).classList.remove(
            "active-piece"
          );
  ...
  for (let i = 0; i < height; i++) {
    for (let j = 0; j < width; j++) {
      if (col > 0) {
        if (
          piece.shape[i][j] === 1 &&
          getElement(`cell-${row + i}-${col - 1}`).classList.contains("taken")
        ) {
          return false;
        }
      }
  ...
  for (let i = 0; i < height; i++) {
    for (let j = 0; j < width; j++) {
      if (col + width < 10) {
        if (
          piece.shape[i][j] === 1 &&
          getElement(`cell-${row + i}-${col + width}`).classList.contains(
            "taken"
          )
        ) {
          return false;
    ...
    if (row + height >= 20) {
      ...
      for (let i = 0; i < height; i++) {
        for (let j = 0; j < width; j++) {
          if (shape[i][j] === 1) {
            if (
              getElement(`cell-${row + i + 1}-${col + j}`).classList.contains(
                "taken"
              )
            ) {
              return true;
